...
$show_breadcrumb = sp_get_option( 'product_category_breadcrumbs' );

// bail if show breadcrumb is not on
...
$delimiter	= '<i class="icon-angle-right separator"></i>';

// If permalinks contain the shop page in the URI prepend the breadcrumb with shop
if ( $shop_page_id && strstr( $permalinks['product_base'], $delimiter . $shop_page->post_name ) && get_option( 'page_on_front' ) !== $shop_page_id ) {
...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == woocommerce_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( ! empty( $home ) ) {
		echo $before . '<a class="home" href="' . apply_filters( 'woocommerce_breadcrumb_home_url', home_url() ) . '"><i class="icon-home"></i></a>' . $after . $delimiter;
	}
	...
	if ( is_category() ) {
		...
		echo $prepend . $before . __( 'Products tagged &ldquo;', 'sp-theme' ) . $queried_object->name . '&rdquo;' . $after;
		...
		if ( is_search() ) {
			...
			echo $before . '<a href="' . get_post_type_archive_link('product') . '">' . $_name . '</a>' . $delimiter . __( 'Search results for &ldquo;', 'sp-theme' ) . get_search_query() . '&rdquo;' . $after;
		...
		echo $before . __( 'Error 404', 'sp-theme' ) . $after;
		...
		echo $before . __( 'Search results for &ldquo;', 'sp-theme' ) . get_search_query() . '&rdquo;' . $after;

	} elseif ( is_tag() ) {

			echo $before . __( 'Posts tagged &ldquo;', 'sp-theme' ) . single_tag_title('', false) . '&rdquo;' . $after;

	} elseif ( is_author() ) {

		$userdata = get_userdata($author);
		echo $before . __( 'Author:', 'sp-theme' ) . ' ' . $userdata->display_name . $after;
