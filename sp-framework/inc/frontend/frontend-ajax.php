...
function sp_submit_contact_form_ajax() {
	...
	foreach( $form_data as $k => $v ) {
		...
		if ( is_array( $v ) ) {
			$items = implode( ',', $v );
			$email_template = str_replace( '[' . $k . ']', $items, $email_template );
		} else {
    ...
    $headers = sprintf( __( '%s', 'sp-theme' ) . ' ' . wp_specialchars_decode( get_bloginfo( 'name' ), ENT_QUOTES ) . ' | ' . $header_text . ' <' . $from_email . '>', 'From:' ) . PHP_EOL;  
